...
SELECT id, name, lng, lat, alt{{ if isSet "lat" }}{{if isSet "lng" }}, (ROUND(ST_DistanceSphere(the_geom, ST_GeomFromText('POINT({{ .lng }} {{ .lat }})', 4326)) / 10) / 100) AS distance_km{{ end }}{{ end }}
